...
from .models import Sis_produto, produtos_schema, produto_schema
...
def create(produto):
    ...
    existing_produto = Sis_produto.query.filter(Sis_produto.nome == nome).one_or_none()
...
def read_one(id):
    produto = Sis_produto.query.filter(Sis_produto.id == id).one_or_none()
...
def update(id, produto):
    existing_produto = Sis_produto.query.filter(Sis_produto.id == id).one_or_none()
